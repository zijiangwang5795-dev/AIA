...
> v5.2 · 2026 · OpenClaw 集成版

| 模块 | 技术选型 |
|------|---------|
| **前端** | Android WebView + HTML/JS |
| **后端** | Node.js · Fastify |
| **AI 网关** | OpenClaw（自部署，端口 18789） |
| **数据库** | PostgreSQL + pgvector + Redis |
| **AI 模型** | DeepSeek · GPT-4o-mini · Claude · Ollama（由 OpenClaw 管理） |
| **语音** | Android SpeechRecognizer → Whisper |
| **架构模式** | 前端 → 后端 → **OpenClaw** → 大模型 · 两层大脑 |

*包含：助手人格层 · 两层大脑架构 · OpenClaw AI 网关 · 语音全链路 · 实时流式输出 · 客户端技能分发 · 双身份系统*
...
| `done` | 后端→前端 | `{type, runId, totalTokens, latencyMs, costUsd, model, gateway}` | 全流程结束（`gateway: "openclaw"` 表示已走网关） |
...
AI 助手采用「前端 → 后端 → **OpenClaw 网关** → 大模型」四层架构。核心创新在于后端内的「两层大脑」设计，以及将所有 AI 推理统一经由 OpenClaw 网关路由，实现模型无关性与灵活切换。
...
│                         │  组装好的完整上下文 + 模型 hint        │
│  ┌──────────────────────▼───────────────────────────────────┐   │
│  │         🤖 第二层大脑：Agent ReAct 循环 (流式 SSE)        │   │
│  │         · 调用 OpenClaw  · Tool 执行  · 流式生成          │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                  │
│  助手人格层  [灵魂 Soul] + [天赋 Talent] + [技能 Skill]          │
│  知识库层   [向量库 pgvector] + [结构化 DB] + [文档 RAG]         │
└────────────────────────┬─────────────────────────────────────────┘
                         │  HTTP /v1/chat/completions (流式)
┌────────────────────────▼─────────────────────────────────────────┐
│              🦞 OpenClaw 网关  (服务器部署，端口 18789)           │
│                                                                  │
│  · 统一管理所有 AI Keys（后端不直接持有 Provider Keys）          │
│  · 模型路由：按名称分发到对应 Provider                           │
│  · 支持流式 SSE 透传                                             │
│  · 内置会话管理、工具调用、浏览器自动化等能力                    │
└────────────────────────┬─────────────────────────────────────────┘
                         │
          ┌──────────────┼──────────────────┐
          ▼              ▼                  ▼
    DeepSeek API    OpenAI API          Claude API / Ollama（本地）
```
...
| AI 统一走网关 | 所有推理请求经 OpenClaw，后端不直连 Provider | 模型无关、Key 统一管理、易切换 |
| Key 不出后端 | Provider API Keys 只在 OpenClaw 侧配置 | 安全，防止 Key 滥用泄漏 |
...
> OpenClaw 是自部署的开源 AI 代理运行时，提供 OpenAI 兼容 API，统一管理模型调用、流式输出和工具扩展。
> 项目地址：[github.com/openclaw/openclaw](https://github.com/openclaw/openclaw)

**集成方式：**

后端通过 `src/brain/openclaw/client.js` 与 OpenClaw 通信。该模块封装了 OpenAI SDK，将 `baseURL` 指向 OpenClaw 网关地址。

```
后端 callOpenClaw()
    ↓
POST http://<OPENCLAW_HOST>:18789/v1/chat/completions
    { model, messages, tools, stream: true }
    ↓
OpenClaw 路由至对应 Provider（DeepSeek / OpenAI / Claude / Ollama）
    ↓
SSE 流式返回 text chunks + tool_calls
    ↓
后端中继给前端 SSE 连接
```
...
| 变量 | 说明 | 默认值 |
|------|------|--------|
| `OPENCLAW_URL` | OpenClaw 服务地址 | `http://localhost:18789` |
| `OPENCLAW_TOKEN` | 认证 token（auth.mode 启用时） | _(空)_ |
| `OPENCLAW_DEFAULT_MODEL` | OpenClaw 内默认路由模型 | `deepseek-chat` |
| `OPENCLAW_EMBED_MODEL` | 向量化模型名 | `text-embedding-3-small` |

**迁移到 OpenClaw 的 AI 功能：**

| 功能 | 迁移前 | 迁移后 |
|------|--------|--------|
| Agent 主推理（ReAct） | 直接调 DeepSeek/OpenAI | 经 OpenClaw `/v1/chat/completions` |
| 情节记忆摘要生成 | 直接调 DeepSeek/OpenAI | 经 OpenClaw |
...
│   │   │   ├── intentClassifier.ts
│   │   │   ├── contextAssembler.ts
│   │   │   └── router.ts    # 模型路由决策（产出 model hint）
│   │   ├── layer2/          # 第二层：大模型推理
│   │   │   ├── agentExecutor.ts  # ReAct 循环（调用 OpenClaw）
│   │   │   └── streamAdapter.ts  # SSE 流式适配
│   │   └── openclaw/        # OpenClaw 网关客户端
│   │       └── client.ts    # callOpenClaw / createEmbedding
│   │
│   ├── tools/               # Tool 注册表
│   │   ├── registry.ts
│   │   ├── webSearch.ts
...
| API Key | 只在 OpenClaw 侧配置，后端不直持 | 安全，Provider Keys 集中管理，切换模型无需重启后端 |
