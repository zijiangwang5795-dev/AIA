...
后端通过 `src/brain/openclaw/client.js` 与 OpenClaw 通信，封装了 OpenAI SDK 并将 `baseURL` 指向网关地址。

```
后端 callOpenClaw({ ..., userId })
    ↓ resolveUserConfig(userId)  ← 按模式解析目标实例
    ↓
POST http://<OPENCLAW_HOST>:18789/v1/chat/completions
    { model, messages, tools, stream: true, user: "aia_{userId}" }
    ↓
